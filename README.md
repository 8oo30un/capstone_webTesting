...
- **원인**: 페이지 스크롤로 인해 화면에 보이는 요소만 인식하여 DOM 순서와 무관하게 클릭
- **해결 방법**:
  - 각 사용자 선택 전 페이지를 최상단으로 스크롤
  - DOM의 `nth-child` 셀렉터 사용하여 순차적으로 1, 2, 3, 4번째 행 클릭
  - 화면 표시 여부와 관계없이 DOM 전체에서 순서 보장

**✅ 수정 후 검증된 순서 (TOS 내림차순):**
